...
describe 'security' do
  describe 'session fixation' do
    it 'changes the cookie session id after logging in' do
      u =
        user_with_pseudonym active_user: true, username: 'nobody@example.com', password: 'asdfasdf'
      u.save!
      ...
      post '/login/canvas',
           params: {
             'pseudonym_session[unique_id]' => 'nobody@example.com',
  ...
  describe 'remember me' do
    before do
      @u =
        user_with_pseudonym active_all: true, username: 'nobody@example.com', password: 'asdfasdf'
      @u.save!
      @p = @u.pseudonym
      https!
    end
    ...
    it 'does not remember me when the wrong token is given' do
      ...
      get '/',
          headers: {
            'HTTP_COOKIE' => "pseudonym_credentials=#{token.id}::#{@p.persistence_token}::blah"
          }
      expect(response).to redirect_to('https://www.example.com/login')
    end
    ...
    it 'destroys the token both user agent and server side on logout' do
      ...
      expect { delete '/logout' }.to change(SessionPersistenceToken, :count).by(-1)
    ...
    context 'sharding' do
      ...
      it 'works for an out-of-shard user' do
        ...
        get '/',
            headers: {
              'HTTP_COOKIE' => "pseudonym_credentials=#{token.pseudonym_credentials}"
            }
        expect(response).to be_successful
        expect(cookies['_normandy_session']).to be_present
        expect(session[:used_remember_me_token]).to be_truthy
    ...
    describe 'max login attempts' do
      ...
      def bad_login(ip)
        post '/login/canvas',
             params: {
               'pseudonym_session[unique_id]' => 'nobody@example.com',
               'pseudonym_session[password]' => 'failboat'
             },
             headers: {
               'REMOTE_ADDR' => ip
             }
      ...
      it 'does not block other users with the same ip' do
        ...
        user_with_pseudonym(active_user: true, username: 'second@example.com', password: '12341234')
          .save!
        post '/login/canvas',
             params: {
               'pseudonym_session[unique_id]' => 'second@example.com',
               'pseudonym_session[password]' => '12341234'
             },
             headers: {
               'REMOTE_ADDR' => '5.5.5.5'
             }
        follow_redirect! while response.redirect?
        expect(request.fullpath).to eql('/login/canvas')
      end

      it 'applies limitations correctly for cross-account logins' do
        account = Account.create!
        allow_any_instantiation_of(Account.default).to receive(:trusted_account_ids).and_return(
          [account.id]
        )
    ...
    expect(json_parse).to eq(
      {
        'duplicates' => [],
        'errored_users' => [
          { 'address' => 'A345678', 'details' => 'not_found', 'type' => 'pseudonym' }
        ],
        'users' => [
          {
            'address' => 'A1234567',
  ...
  describe 'user masquerading' do
    ...
    it 'remembers the destination with an intervening auth' do
      ...
      post '/login/canvas',
           params: {
             pseudonym_session: {
               unique_id: @admin.pseudonyms.first.unique_id,
               password: 'password'
             }
           }
      expect(response).to redirect_to user_masquerade_url(@student)
  ...
  describe 'admin permissions' do
    ...
    describe 'root account' do
      ...
      it 'manage_user_notes' do
        ...
        @user_note =
          UserNote.create!(creator: @teacher, user: @student, root_account_id: Account.default.id)
