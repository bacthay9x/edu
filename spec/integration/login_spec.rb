...
describe 'login' do
  ...
  context 'cas' do
    before { account_with_cas(account: Account.default) }

    def stubby(stub_response)
      @cas_client =
        CASClient::Client.new(
    ...
    context 'single sign out' do
      before { skip 'needs redis' unless Canvas.redis_enabled? }
  ...
  context 'SAML' do
    before { skip('requires SAML extension') unless AuthenticationProvider::SAML.enabled? }
