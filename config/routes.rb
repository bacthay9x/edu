...
CanvasRails::Application.routes.draw do
  ...
  resources :submission_comments, only: %i[update destroy]
  ...
  get 'conversations/unread' => 'conversations#index',
      :as => :conversations_unread,
      :redirect_scope => 'unread'
  get 'conversations/starred' => 'conversations#index',
      :as => :conversations_starred,
      :redirect_scope => 'starred'
  get 'conversations/sent' => 'conversations#index',
      :as => :conversations_sent,
      :redirect_scope => 'sent'
  get 'conversations/archived' => 'conversations#index',
      :as => :conversations_archived,
      :redirect_scope => 'archived'
  ...
  match 'register/:nonce' => 'communication_channels#confirm',
        :as => :registration_confirmation,
        :via => %i[get post]

  # deprecated
  get 'pseudonyms/:id/register/:nonce' => 'communication_channels#confirm',
      :as => :registration_confirmation_deprecated
  post 'confirmations/:user_id/re_send(/:id)' => 'communication_channels#re_send_confirmation',
       :as => :re_send_confirmation,
       :id => nil
  get 'confirmations/:user_id/limit_reached(/:id)' =>
        'communication_channels#confirmation_limit_reached',
      :as => :confirmation_limit_reached,
      :id => nil
  match 'forgot_password' => 'pseudonyms#forgot_password',
        :as => :forgot_password,
        :via => %i[get post]
  get 'pseudonyms/:pseudonym_id/change_password/:nonce' => 'pseudonyms#confirm_change_password',
      :as => :confirm_change_password
  post 'pseudonyms/:pseudonym_id/change_password/:nonce' => 'pseudonyms#change_password',
       :as => :change_password
  ...
  concern :files do
    resources :files, except: [:new] do
      ...
      collection do
        get "folder#{full_path_glob}" => 'files#react_files',
            :format => false,
            :defaults => {
              format: 'html'
            }
  ...
  concern :relative_files do
    get 'file_contents/*file_path' => 'files#show_relative',
  ...
  concern :announcements do
    ...
    delete 'announcements/external_feeds/:id' => 'announcements#destroy_external_feed',
           :as => :announcements_external_feed
  end
  ...
  resources :courses do
    ...
    post 'self_unenrollment/:self_unenrollment' => 'courses#self_unenrollment',
         :as => :self_unenrollment
    post :unconclude
    get :students
    get 'observer_pairing_codes.csv',
        action: :observer_pairing_codes_csv,
        as: 'observer_pairing_codes'
    ...
    resources :sections, except: %i[index edit new] do
      get 'crosslist/confirm/:new_course_id' => 'sections#crosslist_check',
          :as => :confirm_crosslist
      post :crosslist
    ...
    resource :gradebook do
      get 'submissions_upload/:assignment_id' => 'gradebooks#show_submissions_upload',
          :as => :show_submissions_upload
      post 'submissions_upload/:assignment_id' => 'gradebooks#submissions_zip_upload',
           :as => :submissions_upload
    ...
    post 'save_assignment_order' => 'gradebooks#save_assignment_order',
         :as => :save_assignment_order
    ...
    resources :assignments do
      ...
      get 'anonymous_submissions/:anonymous_id',
          ...
          constraints:
            lambda { |request| request.query_parameters.key?(:preview) && request.format == :html }

      get 'anonymous_submissions/:anonymous_id',
          to: 'submissions/anonymous_downloads#show',
          constraints: lambda { |request| request.query_parameters.key?(:download) }

      get 'anonymous_submissions/:anonymous_id',
          to: 'anonymous_submissions#show',
          as: :anonymous_submission
      ...
      put 'anonymous_submissions/:anonymous_id/reassign',
          to: 'anonymous_submissions#redo_submission'
      resources :submissions do
        get 'originality_report/:asset_string' => 'submissions#originality_report',
            :as => :originality_report
      ...
      post 'peer_reviews/users/:reviewer_id' => 'assignments#assign_peer_review',
           :as => :assign_peer_review
    ...
    resources :external_tools do
      match :resource_selection, via: %i[get post]
      get :homework_submission
      get :finished
      collection { get :retrieve }
    end
    ...
    post 'lti/tool_proxy_registration',
         controller: 'lti/message',
         action: 'registration',
         as: :tool_proxy_registration
    ...
    resources :quizzes, controller: 'quizzes/quizzes', except: :new do
      ...
      resources :quiz_submissions, controller: 'quizzes/quiz_submissions', path: :submissions do
        ...
        resources :events,
                  controller: 'quizzes/quiz_submission_events',
                  path: "log#{full_path_glob}"
      end
      ...
      match 'take' => 'quizzes/quizzes#show', :take => '1', :via => %i[get post]
    ...
    resources :context_modules, path: :modules do
      ...
      get 'prerequisites/:code' => 'context_modules#content_tag_prerequisites_needing_finishing',
          :as => :prerequisites_needing_finishing
    ...
    get 'modules/items/assignment_info' => 'context_modules#content_tag_assignment_data',
        :as => :context_modules_assignment_info
    get 'modules/items/master_course_info' => 'context_modules#content_tag_master_course_data',
        :as => :context_modules_master_course_info
    get 'modules/items/:id' => 'context_modules#item_redirect',
        :as => :context_modules_item_redirect
    ...
    get 'modules/items/sequence/:id' => 'context_modules#item_details',
        :as => :context_modules_item_details
  ...
  get 'quiz_statistics/:quiz_statistics_id/files/:file_id/download' => 'files#show',
      :as => :quiz_statistics_download,
      :download => '1'
  ...
  get 'media_objects/:id/redirect' => 'media_objects#media_object_redirect',
      :as => :media_object_redirect
  get 'media_objects/:id/thumbnail' => 'media_objects#media_object_thumbnail',
      :as => :media_object_thumbnail
  get 'media_objects/:media_object_id/info' => 'media_objects#show', :as => :media_object_info
  get 'media_objects_iframe/:media_object_id' => 'media_objects#iframe_media_player',
      :as => :media_object_iframe
  get 'media_objects_iframe' => 'media_objects#iframe_media_player',
      :as => :media_object_iframe_href
  get 'media_objects/:media_object_id/media_tracks/:id' => 'media_tracks#show',
      :as => :show_media_tracks
  post 'media_objects/:media_object_id/media_tracks' => 'media_tracks#create',
       :as => :create_media_tracks
  delete 'media_objects/:media_object_id/media_tracks/:media_track_id' => 'media_tracks#destroy',
         :as => :delete_media_tracks

  get 'external_content/success/:service' => 'external_content#success',
      :as => :external_content_success
  get 'external_content/success/:service/:id' => 'external_content#success',
      :as => :external_content_update
  get 'external_content/retrieve/oembed' => 'external_content#oembed_retrieve',
      :as => :external_content_oembed_retrieve
  get 'external_content/cancel/:service' => 'external_content#cancel',
      :as => :external_content_cancel

  %w[account course group].each do |context|
    prefix = "#{context}s/:#{context}_id"
    post "#{prefix}/deep_linking_response",
         controller: 'lti/ims/deep_linking',
         action: :deep_linking_response,
         as: "#{context}_deep_linking_response"
  end

  %w[account course group user].each do |context|
    match "#{context.pluralize}/:#{context}_id/external_content/success/:service" =>
            'external_content#success',
          :as => "#{context}_external_content_success",
          :via => %i[get post]
    match "#{context.pluralize}/:#{context}_id/external_content/success/:service/:id" =>
            'external_content#success',
          :as => "#{context}_external_content_update",
          :via => %i[get post]
  end
  ...
  scope '/feeds' do
    ...
    get 'calendars/:feed_code.:format' => 'calendar_events_api#public_feed',
        :as => :feeds_calendar_format
    ...
    get 'topics/:discussion_topic_id/:feed_code' => 'discussion_entries#public_feed',
        :as => :feeds_topic
    get 'topics/:discussion_topic_id/:feed_code.:format' => 'discussion_entries#public_feed',
        :as => :feeds_topic_format
    get 'announcements/:feed_code' => 'announcements#public_feed', :as => :feeds_announcements
    get 'announcements/:feed_code.:format' => 'announcements#public_feed',
        :as => :feeds_announcements_format
    ...
    get 'conversations/:feed_code.:format' => 'conversations#public_feed',
        :as => :feeds_conversation_format
  end
  ...
  resources :assessment_questions do
    ...
    get 'files/:id/:verifier' => 'files#assessment_question_show',
        :as => :verified_file,
        :download => '1'
  end
  ...
  resources :eportfolios, except: :index do
    ...
    post ':eportfolio_category_id/reorder_entries' => 'eportfolios#reorder_entries',
         :as => :reorder_entries
    resources :categories, controller: :eportfolio_categories
    resources :entries, controller: :eportfolio_entries do
      resources :page_comments, path: :comments, only: %i[create destroy]
      get 'files/:attachment_id' => 'eportfolio_entries#attachment', :as => :view_file
      get 'submissions/:submission_id' => 'eportfolio_entries#submission',
          :as => :preview_submission
    end
  ...
  resources :accounts do
    ...
    get 'statistics/over_time/:attribute.:format' => 'accounts#statistics_graph',
        :as => :formatted_statistics_graph
    get :turnitin_confirmation
    get :vericite_confirmation
    resources :permissions, controller: :role_overrides, only: %i[index create] do
      collection do
        post :add_role
        delete :remove_role
      end
    ...
    resources :role_overrides, only: %i[index create] do
      collection do
        post :add_role
        delete :remove_role
      end
    ...
    concerns :announcements
    resources :submissions
    delete 'authentication_providers' => 'authentication_providers#destroy_all',
           :as => :remove_all_authentication_providers
    ...
    match 'test_ldap_logins' => 'authentication_providers#test_ldap_login', :via => %i[get post]
  ...
  get 'login/saml_idp_discovery/:id' => 'login/saml_idp_discovery#new',
      :as => :saml_idp_discovery_login
  ...
  delete 'users/self/otps' => 'one_time_passwords#destroy_all',
         :as => :destroy_all_one_time_passwords
  ...
  resources :users, except: %i[destroy index] do
    match 'masquerade', via: %i[get post]
    ...
    get 'teacher_activity/course/:course_id' => 'users#teacher_activity',
        :as => :course_teacher_activity
    get 'teacher_activity/student/:student_id' => 'users#teacher_activity',
        :as => :student_teacher_activity
    get :media_download
  ...
  scope '/dashboard' do
    ...
    delete 'account_notifications/:id' => 'users#close_notification',
           :as => :dashboard_close_notification
    get 'eportfolios' => 'eportfolios#user_index', :as => :dashboard_eportfolios
    post 'comment_session' => 'services_api#start_kaltura_session',
         :as => :dashboard_comment_session
    delete 'ignore_stream_item/:id' => 'users#ignore_stream_item',
           :as => :dashboard_ignore_stream_item
  end
  ...
  get 'course_sections/:course_section_id/calendar_events/:id' => 'calendar_events#show',
      :as => :course_section_calendar_event
  get 'files' => 'files#index'
  get "files/folder#{full_path_glob}",
      controller: 'files',
      action: 'react_files',
      format: false,
      defaults: {
  ...
  resources :jobs_v1, controller: :jobs, only: %i[index show] do
    collection { post 'batch_update' }
  end
  ...
  get 'courses/:course_id/outcome_rollups' => 'outcome_results#rollups',
      :as => 'course_outcome_rollups'
  ...
  ApiRouteSet::V1.draw(self) do
    scope(controller: :courses) do
      ...
      get 'courses/:course_id/recent_students',
          action: :recent_students,
          as: 'course_recent_students'
      get 'courses/:course_id/users', action: :users, as: 'course_users'
      get 'courses/:course_id/collaborations',
          controller: :collaborations,
          action: :api_index,
          as: 'course_collaborations_index'
      delete 'courses/:course_id/collaborations/:id', controller: :collaborations, action: :destroy
      put 'courses/:id/quizzes',
          action: 'new_quizzes_selection_update',
          as: 'course_new_quizzes_selection_update'
      post 'courses/:id/dismiss_migration_limitation_message',
           action: 'dismiss_migration_limitation_msg',
           as: 'course_dismiss_migration_limitation_msg'
      ...
      get 'courses/:course_id/content_share_users',
          action: :content_share_users,
          as: 'course_content_share_users'
      get 'courses/:course_id/activity_stream',
          action: :activity_stream,
          as: 'course_activity_stream'
      get 'courses/:course_id/activity_stream/summary',
          action: :activity_stream_summary,
          as: 'course_activity_stream_summary'
      get 'courses/:course_id/bulk_user_progress',
          action: :bulk_user_progress,
          as: 'course_bulk_user_progress'
      ...
      post 'courses/:course_id/course_copy',
           controller: :content_imports,
           action: :copy_course_content
      get 'courses/:course_id/course_copy/:id',
          controller: :content_imports,
          action: :copy_course_status,
          as: :course_copy_status
      ...
      get 'courses/:course_id/folders/by_path/*full_path',
          controller: :folders,
          action: :resolve_path
      ...
      get 'courses/:course_id/media_objects',
          controller: 'media_objects',
          action: :index,
          as: :course_media_objects
      get 'groups/:group_id/media_objects',
          controller: 'media_objects',
          action: :index,
          as: :group_media_objects
      ...
      get 'courses/:course_id/effective_due_dates',
          action: :effective_due_dates,
          as: 'course_effective_due_dates'
    ...
    scope(controller: :account_calendars_api) do
      ...
      put 'accounts/:account_id/account_calendars',
          action: :bulk_update,
          as: :bulk_update_account_calendars
      get 'accounts/:account_id/account_calendars',
          action: :all_calendars,
          as: :all_account_calendars
      get 'accounts/:account_id/visible_calendars_count',
          action: :visible_calendars_count,
          as: :visible_calendars_count
    end
    ...
    scope(controller: :account_notifications) do
      ...
      put 'accounts/:account_id/account_notifications/:id',
          action: :update,
          as: 'account_notification_update'
      get 'accounts/:account_id/account_notifications',
          action: :user_index,
          as: 'user_account_notifications' # to change the api docs
      get 'accounts/:account_id/users/:user_id/account_notifications',
          action: :user_index_deprecated # for back compat
      get 'accounts/:account_id/account_notifications/:id',
          action: :show,
          as: 'user_account_notification_show'
      get 'accounts/:account_id/users/:user_id/account_notifications/:id', action: :show_deprecated
      delete 'accounts/:account_id/account_notifications/:id',
             action: :user_close_notification,
             as: 'user_account_notification'
      delete 'accounts/:account_id/users/:user_id/account_notifications/:id',
             action: :user_close_notification_deprecated
    end

    scope(controller: :brand_configs_api) { get 'brand_variables', action: :show }
    ...
    scope(controller: :scopes_api) { get 'accounts/:account_id/scopes', action: :index }
    ...
    scope(controller: :authentication_audit_api) do
      get 'audit/authentication/logins/:login_id',
          action: :for_login,
          as: 'audit_authentication_login'
      get 'audit/authentication/accounts/:account_id',
          action: :for_account,
          as: 'audit_authentication_account'
    ...
    scope(controller: :grade_change_audit_api) do
      get 'audit/grade_change/assignments/:assignment_id',
          action: :for_assignment,
          as: 'audit_grade_change_assignment'
      get 'audit/grade_change/courses/:course_id',
          action: :for_course,
          as: 'audit_grade_change_course'
      get 'audit/grade_change/students/:student_id',
          action: :for_student,
          as: 'audit_grade_change_student'
      get 'audit/grade_change/graders/:grader_id',
          action: :for_grader,
          as: 'audit_grade_change_grader'
    ...
    scope(controller: :assignment_overrides) do
      ...
      get 'courses/:course_id/assignments/:assignment_id/overrides/:id',
          action: :show,
    ...
    scope(controller: :assignments_api) do
      get 'courses/:course_id/assignments/gradeable_students',
          controller: :submissions_api,
          action: :multiple_gradeable_students,
          as: 'multiple_assignments_gradeable_students'
      get 'courses/:course_id/assignments', action: :index, as: 'course_assignments'
      get 'courses/:course_id/assignment_groups/:assignment_group_id/assignments',
          action: :index,
          as: 'course_assignment_group_assignments'
      get 'users/:user_id/courses/:course_id/assignments',
          action: :user_index,
          as: 'user_course_assignments'
    ...
    scope(controller: 'assignment_extensions') do
      post 'courses/:course_id/assignments/:assignment_id/extensions',
           action: :create,
           as: 'course_assignment_extensions_create'
    end
    ...
    scope(controller: :peer_reviews_api) do
      ...
      get 'courses/:course_id/assignments/:assignment_id/submissions/:submission_id/peer_reviews',
          action: :index
      get 'sections/:section_id/assignments/:assignment_id/submissions/:submission_id/peer_reviews',
          action: :index
      post 'courses/:course_id/assignments/:assignment_id/submissions/:submission_id/peer_reviews',
           action: :create
      post 'sections/:section_id/assignments/:assignment_id/submissions/:submission_id/peer_reviews',
           action: :create
      delete 'courses/:course_id/assignments/:assignment_id/submissions/:submission_id/peer_reviews',
             action: :destroy
      delete 'sections/:section_id/assignments/:assignment_id/submissions/:submission_id/peer_reviews',
             action: :destroy
    end

    scope(controller: :moderation_set) do
      get 'courses/:course_id/assignments/:assignment_id/moderated_students',
          action: :index,
          as: :moderated_students
      post 'courses/:course_id/assignments/:assignment_id/moderated_students',
           action: :create,
           as: :add_moderated_students
    end
    ...
    scope(controller: :submissions_api) do
      [%w[course course], %w[section course_section]].each do |(context, path_prefix)|
        ...
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/read",
            action: :mark_submission_read,
            as: "#{context}_submission_mark_read"
        delete "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/read",
               action: :mark_submission_unread,
               as: "#{context}_submission_mark_unread"
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/read/:item",
            action: :mark_submission_item_read,
            as: "#{context}_submission_mark_item_read"
        put "#{context.pluralize}/:#{context}_id/submissions/bulk_mark_read",
            action: :mark_bulk_submissions_as_read,
            as: "#{context}_submissions_bulk_mark_read"
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/document_annotations/read",
            action: :document_annotations_read_state,
            as: "#{path_prefix}_submission_document_annotations_read_state"
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/document_annotations/read",
            action: :mark_document_annotations_read,
            as: "#{path_prefix}_submission_document_annotations_mark_read"
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/rubric_comments/read",
            action: :rubric_assessments_read_state,
            as: "#{path_prefix}_submission_rubric_comments_read_state"
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/rubric_comments/read",
            action: :mark_rubric_assessments_read,
            as: "#{path_prefix}_submission_rubric_comments_mark_read"
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/rubric_assessments/read",
            action: :rubric_assessments_read_state,
            as: "#{path_prefix}_submission_rubric_assessments_read_state"
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/rubric_assessments/read",
            action: :mark_rubric_assessments_read,
            as: "#{path_prefix}_submission_rubric_assessments_mark_read"
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions",
            action: :index,
            as: "#{path_prefix}_assignment_submissions"
        get "#{context.pluralize}/:#{context}_id/students/submissions",
            controller: :submissions_api,
            action: :for_students,
            as: "#{path_prefix}_student_submissions"
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id",
            action: :show,
            as: "#{path_prefix}_assignment_submission"
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/anonymous_submissions/:anonymous_id",
            action: :show_anonymous
        post "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions",
             action: :create,
             controller: :submissions
        post "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id/files",
             action: :create_file
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/:user_id",
            action: :update
        put "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/anonymous_submissions/:anonymous_id",
            action: :update_anonymous
        post "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submissions/update_grades",
             action: :bulk_update
        get "#{context.pluralize}/:#{context}_id/assignments/:assignment_id/submission_summary",
            action: :submission_summary,
            as: "#{path_prefix}_assignment_submission_summary"
      end
      get 'courses/:course_id/assignments/:assignment_id/gradeable_students',
          action: :gradeable_students,
          as: 'course_assignment_gradeable_students'
    end
    ...
    scope(controller: :submission_comments_api) do
      post '/courses/:course_id/assignments/:assignment_id/submissions/:user_id/comments/files',
           action: :create_file
      put 'courses/:course_id/assignments/:assignment_id/submissions/:user_id/comments/:id',
          action: :update
      delete 'courses/:course_id/assignments/:assignment_id/submissions/:user_id/comments/:id',
             action: :destroy
    end

    post '/courses/:course_id/assignments/:assignment_id/submissions/:user_id/annotation_notification',
    ...
    scope(controller: :gradebook_history_api) do
      ...
      get 'courses/:course_id/gradebook_history/:date',
          action: :day_details,
          as: 'gradebook_history_for_day'
      get 'courses/:course_id/gradebook_history/:date/graders/:grader_id/assignments/:assignment_id/submissions',
          action: :submissions,
          as: 'gradebook_history_submissions'
    end
    ...
    scope(controller: :content_migrations) do
      %w[account course group user].each do |context|
        get "#{context.pluralize}/:#{context}_id/content_migrations/migrators",
            action: :available_migrators,
            as: "#{context}_content_migration_migrators_list"
        get "#{context.pluralize}/:#{context}_id/content_migrations/:id",
            action: :show,
            as: "#{context}_content_migration"
        get "#{context.pluralize}/:#{context}_id/content_migrations",
            action: :index,
            as: "#{context}_content_migration_list"
        post "#{context.pluralize}/:#{context}_id/content_migrations",
             action: :create,
             as: "#{context}_content_migration_create"
        put "#{context.pluralize}/:#{context}_id/content_migrations/:id",
            action: :update,
            as: "#{context}_content_migration_update"
        get "#{context.pluralize}/:#{context}_id/content_migrations/:id/selective_data",
            action: :content_list,
            as: "#{context}_content_migration_selective_data"
      end
    ...
    scope(controller: :migration_issues) do
      %w[account course group user].each do |context|
        get "#{context.pluralize}/:#{context}_id/content_migrations/:content_migration_id/migration_issues/:id",
            action: :show,
            as: "#{context}_content_migration_migration_issue"
        get "#{context.pluralize}/:#{context}_id/content_migrations/:content_migration_id/migration_issues",
            action: :index,
            as: "#{context}_content_migration_migration_issue_list"
        post "#{context.pluralize}/:#{context}_id/content_migrations/:content_migration_id/migration_issues",
             action: :create,
             as: "#{context}_content_migration_migration_issue_create"
        put "#{context.pluralize}/:#{context}_id/content_migrations/:content_migration_id/migration_issues/:id",
            action: :update,
            as: "#{context}_content_migration_migration_issue_update"
      end
    end

    scope(controller: :discussion_topics_api) do
      %w[course group].each do |context|
        get "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id",
            action: :show,
            as: "#{context}_discussion_topic"
        post "#{context.pluralize}/:#{context}_id/discussion_topics",
             controller: :discussion_topics,
             action: :create
        put "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id",
            controller: :discussion_topics,
            action: :update
        post "#{context.pluralize}/:#{context}_id/discussion_topics/reorder",
             controller: :discussion_topics,
             action: :reorder
        delete "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id",
               controller: :discussion_topics,
               action: :destroy

        get "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/view",
            action: :view,
            as: "#{context}_discussion_topic_view"
        post "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/duplicate",
             action: :duplicate
        get "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entry_list",
            action: :entry_list,
            as: "#{context}_discussion_topic_entry_list"
        post "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries",
             action: :add_entry,
             as: "#{context}_discussion_add_entry"
        get "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries",
            action: :entries,
            as: "#{context}_discussion_entries"
        post "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries/:entry_id/replies",
             action: :add_reply,
             as: "#{context}_discussion_add_reply"
        get "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries/:entry_id/replies",
            action: :replies,
            as: "#{context}_discussion_replies"
        put "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries/:id",
            controller: :discussion_entries,
            action: :update,
            as: "#{context}_discussion_update_reply"
        delete "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries/:id",
               controller: :discussion_entries,
               action: :destroy,
               as: "#{context}_discussion_delete_reply"

        put "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/read",
            action: :mark_topic_read,
            as: "#{context}_discussion_topic_mark_read"
        delete "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/read",
               action: :mark_topic_unread,
               as: "#{context}_discussion_topic_mark_unread"
        put "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/read_all",
            action: :mark_all_read,
            as: "#{context}_discussion_topic_mark_all_read"
        delete "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/read_all",
               action: :mark_all_unread,
               as: "#{context}_discussion_topic_mark_all_unread"
        put "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries/:entry_id/read",
            action: :mark_entry_read,
            as: "#{context}_discussion_topic_discussion_entry_mark_read"
        delete "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/entries/:entry_id/read",
               action: :mark_entry_unread,
               as: "#{context}_discussion_topic_discussion_entry_mark_unread"
        ...
        put "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/subscribed",
            action: :subscribe_topic,
            as: "#{context}_discussion_topic_subscribe"
        delete "#{context.pluralize}/:#{context}_id/discussion_topics/:topic_id/subscribed",
               action: :unsubscribe_topic,
               as: "#{context}_discussion_topic_unsubscribe"
      end
    end

    scope(controller: :discussion_topic_users) do
      get 'courses/:course_id/discussion_topics/:topic_id/messageable_users',
          action: :search,
          as: 'course_discussion_messageable_users'
      get 'groups/:group_id/discussion_topics/:topic_id/messageable_users',
          action: :search,
          as: 'group_discussion_messageable_users'
    end

    scope(controller: :collaborations) do
      get 'collaborations/:id/members', action: :members, as: 'collaboration_members'
      get 'courses/:course_id/potential_collaborators',
          action: :potential_collaborators,
          as: 'course_potential_collaborators'
      get 'groups/:group_id/potential_collaborators',
          action: :potential_collaborators,
          as: 'group_potential_collaborators'
    end
    ...
    scope(controller: :external_tools) do
      post '/accounts/:account_id/external_tools/rce_favorites/:id',
           action: :add_rce_favorite,
           as: :account_external_tools_add_rce_favorite
      delete '/accounts/:account_id/external_tools/rce_favorites/:id',
             action: :remove_rce_favorite,
             as: :account_external_tools_remove_rce_favorite

      get '/courses/:course_id/external_tools/visible_course_nav_tools',
          action: :visible_course_nav_tools,
          as: :visible_course_nav_tools
      get '/external_tools/visible_course_nav_tools',
          action: :all_visible_nav_tools,
          as: :all_visible_nav_tools

      %w[course account].each do |context|
        get "#{context}s/:#{context}_id/external_tools/sessionless_launch",
            action: :generate_sessionless_launch,
            as: "#{context}_external_tool_sessionless_launch"
        get "#{context}s/:#{context}_id/external_tools/:external_tool_id",
            action: :show,
            as: "#{context}_external_tool_show"
        get "#{context}s/:#{context}_id/external_tools",
            action: :index,
            as: "#{context}_external_tools"
        post "#{context}s/:#{context}_id/external_tools",
             action: :create,
             as: "#{context}_external_tools_create"
        post "#{context}s/:#{context}_id/create_tool_with_verification",
             action: :create_tool_with_verification,
             as: "#{context}_create_tool_with_verification"
        put "#{context}s/:#{context}_id/external_tools/:external_tool_id",
            action: :update,
            as: "#{context}_external_tools_update"
        delete "#{context}s/:#{context}_id/external_tools/:external_tool_id",
               action: :destroy,
               as: "#{context}_external_tools_delete"
      end
    ...
    scope(controller: 'lti/lti_apps') do
      %w[course account].each do |context|
        get "#{context}s/:#{context}_id/lti_apps/launch_definitions",
            action: :launch_definitions,
            as: "#{context}_launch_definitions"
    ...
    scope(controller: :external_feeds) do
      %w[course group].each do |context|
        get "#{context}s/:#{context}_id/external_feeds",
            action: :index,
            as: "#{context}_external_feeds"
        post "#{context}s/:#{context}_id/external_feeds",
             action: :create,
             as: "#{context}_external_feeds_create"
        delete "#{context}s/:#{context}_id/external_feeds/:external_feed_id",
               action: :destroy,
               as: "#{context}_external_feeds_delete"
      end
    end
    ...
    scope(controller: :sis_import_errors_api) do
      get 'accounts/:account_id/sis_imports/:id/errors',
          action: :index,
          as: :sis_batch_import_errors
    ...
    scope(controller: :outcome_imports_api) do
      %w[account course].each do |context|
        post "#{context}s/:#{context}_id/outcome_imports(/group/:learning_outcome_group_id)",
             action: :create
        get "#{context}s/:#{context}_id/outcome_imports/:id", action: :show
        get "#{context}s/:#{context}_id/outcome_imports/:id/created_group_ids",
    ...
    scope(controller: :users) do
      ...
      get 'users/self/activity_stream/summary',
          action: :activity_stream_summary,
          as: 'user_activity_stream_summary'
      ...
      get 'users/:user_id/missing_submissions',
          action: :missing_submissions,
          as: 'user_missing_submissions'
      ...
      put 'users/:id/merge_into/accounts/:destination_account_id/users/:destination_user_id',
          controller: 'users',
          action: 'merge_into'
      ...
      get 'dashboard/dashboard_cards',
          controller: 'users',
          action: 'dashboard_cards',
          as: :dashboard_dashboard_cards

      get 'users/:id/graded_submissions',
          controller: 'users',
      ...
      scope(controller: :observer_alerts_api) do
        ...
        get 'users/:user_id/observer_alerts/:student_id',
            action: :alerts_by_student,
            as: 'observer_alerts_by_student'
      ...
      scope(controller: :observer_alert_thresholds_api) do
        ...
        delete 'users/:user_id/observer_alert_thresholds/:observer_alert_threshold_id',
               action: :destroy
      end
    ...
    scope(controller: :accounts) do
      ...
      get 'accounts/:account_id/courses/:id',
          controller: :courses,
          action: :show,
          as: 'account_course_show'
    ...
    scope(controller: :authentication_providers) do
      ...
      put 'accounts/:account_id/authentication_providers/:id',
          action: :update,
          as: 'account_update_ap'
      delete 'accounts/:account_id/authentication_providers/:id',
             action: :destroy,
             as: 'account_delete_ap'
    end
    ...
    scope(controller: :communication_channels) do
      ...
      post 'users/:user_id/communication_channels/:id',
           action: :reset_bounce_count,
           as: 'reset_bounce_count'
      get 'accounts/:account_id/bounced_communication_channels.csv',
          action: :bouncing_channel_report,
          defaults: {
            format: :csv
          }
      get 'accounts/:account_id/bounced_communication_channels', action: :bouncing_channel_report
      post 'accounts/:account_id/bounced_communication_channels/reset',
           action: :bulk_reset_bounce_counts
      get 'accounts/:account_id/unconfirmed_communication_channels.csv',
          action: :unconfirmed_channel_report,
          defaults: {
            format: :csv
          }
      get 'accounts/:account_id/unconfirmed_communication_channels',
          action: :unconfirmed_channel_report
      ...
      delete 'users/:user_id/communication_channels/:type/:address',
             action: :destroy,
             constraints: {
               address: %r{[^/?]+}
             }
    ...
    scope(controller: :notification_preferences) do
      get 'users/:user_id/communication_channels/:communication_channel_id/notification_preferences',
          action: :index
      get 'users/:user_id/communication_channels/:communication_channel_id/notification_preference_categories',
      ...
      put 'users/self/communication_channels/:communication_channel_id/notification_preference_categories/:category',
          action: :update_preferences_by_category
    end
    ...
    scope(controller: :calendar_events_api) do
      ...
      post 'calendar_events/:id/reservations/:participant_id',
           action: :reserve,
           as: 'calendar_event_reserve'
      get 'calendar_events/:id/participants',
          action: :participants,
          as: 'calendar_event_participants'
      ...
      post 'courses/:course_id/calendar_events/timetable_events',
    ...
    scope(controller: :groups) do
      ...
      get 'groups/:group_id/activity_stream/summary',
          action: :activity_stream_summary,
          as: 'group_activity_stream_summary'
    ...
    scope(controller: :developer_key_account_bindings) do
      post 'accounts/:account_id/developer_keys/:developer_key_id/developer_key_account_bindings',
    ...
    post 'files/:id/create_success',
         controller: :files,
         action: :api_create_success,
         as: 'files_create_success'
    ...
    scope(controller: :favorites) do
      ...
      post 'users/self/favorites/courses/:id',
           ...
           as: :add_favorite_course
      delete 'users/self/favorites/courses/:id',
             action: :remove_favorite_course,
             as: :remove_favorite_course
    ...
    scope(controller: :wiki_pages_api) do
      ...
      get 'courses/:course_id/pages/:url_or_id/revisions',
          action: :revisions,
          as: 'course_wiki_page_revisions'
      get 'groups/:group_id/pages/:url_or_id/revisions',
          action: :revisions,
          as: 'group_wiki_page_revisions'
    ...
    scope(controller: :context_module_items_api) do
      get 'courses/:course_id/modules/:module_id/items',
          action: :index,
          as: 'course_context_module_items'
      get 'courses/:course_id/modules/:module_id/items/:id',
          action: :show,
          as: 'course_context_module_item'
      put 'courses/:course_id/modules/:module_id/items/:id/done',
          action: :mark_as_done,
          as: 'course_context_module_item_done'
      delete 'courses/:course_id/modules/:module_id/items/:id/done',
             action: :mark_as_not_done,
             as: 'course_context_module_item_not_done'
      get 'courses/:course_id/module_item_redirect/:id',
          action: :redirect,
          as: 'course_context_module_item_redirect'
      get 'courses/:course_id/module_item_sequence', action: :item_sequence
      post 'courses/:course_id/modules/:module_id/items',
           action: :create,
           as: 'course_context_module_items_create'
      put 'courses/:course_id/modules/:module_id/items/:id',
          action: :update,
          as: 'course_context_module_item_update'
      ...
      post 'courses/:course_id/modules/:module_id/items/:id/select_mastery_path',
           action: :select_mastery_path
      post 'courses/:course_id/modules/items/:id/duplicate',
           action: :duplicate,
           as: :course_context_module_item_duplicate
    end

    scope(controller: 'quizzes/quiz_assignment_overrides') do
      get 'courses/:course_id/quizzes/assignment_overrides',
          action: :index,
          as: 'course_quiz_assignment_overrides'
      get 'courses/:course_id/new_quizzes/assignment_overrides',
          action: :new_quizzes,
          as: 'course_new_quizzes_assignment_overrides'
    end
    ...
    scope(controller: 'quizzes/quizzes_api') do
      ...
      post 'courses/:course_id/quizzes/:id/validate_access_code',
           action: :validate_access_code,
           as: 'course_quiz_validate_access_code'
    end
    ...
    scope(controller: 'quizzes/quiz_submission_users') do
      get 'courses/:course_id/quizzes/:id/submission_users',
          action: :index,
          as: 'course_quiz_submission_users'
      post 'courses/:course_id/quizzes/:id/submission_users/message',
           action: :message,
           as: 'course_quiz_submission_users_message'
    end

    scope(controller: 'quizzes/quiz_groups') do
      get 'courses/:course_id/quizzes/:quiz_id/groups/:id', action: :show, as: 'course_quiz_group'
      post 'courses/:course_id/quizzes/:quiz_id/groups',
           action: :create,
           as: 'course_quiz_group_create'
      put 'courses/:course_id/quizzes/:quiz_id/groups/:id',
          action: :update,
          as: 'course_quiz_group_update'
      delete 'courses/:course_id/quizzes/:quiz_id/groups/:id',
             action: :destroy,
             as: 'course_quiz_group_destroy'
      post 'courses/:course_id/quizzes/:quiz_id/groups/:id/reorder',
           action: :reorder,
           as: 'course_quiz_group_reorder'
    end

    scope(controller: 'quizzes/quiz_questions') do
      get 'courses/:course_id/quizzes/:quiz_id/questions',
          action: :index,
          as: 'course_quiz_questions'
      get 'courses/:course_id/quizzes/:quiz_id/questions/:id',
          action: :show,
          as: 'course_quiz_question'
      post 'courses/:course_id/quizzes/:quiz_id/questions',
           action: :create,
           as: 'course_quiz_question_create'
      put 'courses/:course_id/quizzes/:quiz_id/questions/:id',
          action: :update,
          as: 'course_quiz_question_update'
      delete 'courses/:course_id/quizzes/:quiz_id/questions/:id',
             action: :destroy,
             as: 'course_quiz_question_destroy'
    end

    scope(controller: 'quizzes/quiz_reports') do
      post 'courses/:course_id/quizzes/:quiz_id/reports',
           action: :create,
           as: 'course_quiz_reports_create'
      delete 'courses/:course_id/quizzes/:quiz_id/reports/:id',
             action: :abort,
             as: 'course_quiz_reports_abort'
    ...
    scope(controller: 'quizzes/quiz_submission_files') do
      post 'courses/:course_id/quizzes/:quiz_id/submissions/self/files',
           action: :create,
           as: 'quiz_submission_files'
    end

    scope(controller: 'quizzes/quiz_submissions_api') do
      get 'courses/:course_id/quizzes/:quiz_id/submission',
          action: :submission,
          as: 'course_quiz_user_submission'
      get 'courses/:course_id/quizzes/:quiz_id/submissions',
          action: :index,
          as: 'course_quiz_submissions'
      get 'courses/:course_id/quizzes/:quiz_id/submissions/:id',
          action: :show,
          as: 'course_quiz_submission'
      get 'courses/:course_id/quizzes/:quiz_id/submissions/:id/time',
          action: :time,
          as: 'course_quiz_submission_time'
      post 'courses/:course_id/quizzes/:quiz_id/submissions',
           action: :create,
           as: 'course_quiz_submission_create'
      put 'courses/:course_id/quizzes/:quiz_id/submissions/:id',
          action: :update,
          as: 'course_quiz_submission_update'
      post 'courses/:course_id/quizzes/:quiz_id/submissions/:id/complete',
           action: :complete,
           as: 'course_quiz_submission_complete'
    end

    scope(controller: 'quizzes/outstanding_quiz_submissions') do
      get 'courses/:course_id/quizzes/:quiz_id/outstanding_quiz_submissions',
          action: :index,
          path_name: 'outstanding_quiz_submission_index'
      post 'courses/:course_id/quizzes/:quiz_id/outstanding_quiz_submissions',
           action: :grade,
           path_name: 'outstanding_quiz_submission_grade'
    end

    scope(controller: 'quizzes/quiz_extensions') do
      post 'courses/:course_id/quizzes/:quiz_id/extensions',
           action: :create,
           as: 'course_quiz_extensions_create'
    end
    ...
    scope(controller: 'quizzes/quiz_submission_events_api') do
      get 'courses/:course_id/quizzes/:quiz_id/submissions/:id/events',
          action: :index,
          as: 'course_quiz_submission_events'
      post 'courses/:course_id/quizzes/:quiz_id/submissions/:id/events',
           action: :create,
           as: 'create_quiz_submission_events'
    end

    scope(controller: 'quizzes/quiz_submission_questions') do
      get '/quiz_submissions/:quiz_submission_id/questions',
          action: :index,
          as: 'quiz_submission_questions'
      post '/quiz_submissions/:quiz_submission_id/questions',
           action: :answer,
           as: 'quiz_submission_question_answer'
      get '/quiz_submissions/:quiz_submission_id/questions/:id/formatted_answer',
          action: :formatted_answer,
          as: 'quiz_submission_question_formatted_answer'
      get '/quiz_submissions/:quiz_submission_id/questions/:id',
          action: :show,
          as: 'quiz_submission_question'
      put '/quiz_submissions/:quiz_submission_id/questions/:id/flag',
          action: :flag,
          as: 'quiz_submission_question_flag'
      put '/quiz_submissions/:quiz_submission_id/questions/:id/unflag',
          action: :unflag,
          as: 'quiz_submission_question_unflag'
    end

    scope(controller: 'quizzes/quiz_ip_filters') do
      get 'courses/:course_id/quizzes/:quiz_id/ip_filters',
          action: :index,
          as: 'course_quiz_ip_filters'
    end

    scope(controller: 'quizzes/quiz_statistics') do
      get 'courses/:course_id/quizzes/:quiz_id/statistics',
          action: :index,
          as: 'course_quiz_statistics'
    end
    ...
    scope(controller: 'polling/poll_submissions') do
      post 'polls/:poll_id/poll_sessions/:poll_session_id/poll_submissions',
           action: :create,
           as: 'poll_submissions_create'
      get 'polls/:poll_id/poll_sessions/:poll_session_id/poll_submissions/:id',
          action: :show,
          as: 'poll_submission'
    end
    ...
    scope(controller: 'live_assessments/assessments') do
      ...
      post 'courses/:course_id/live_assessments',
           action: :create,
           as: 'course_live_assessment_create'
    end

    scope(controller: 'live_assessments/results') do
      get 'courses/:course_id/live_assessments/:assessment_id/results',
          action: :index,
          as: 'course_live_assessment_results'
      post 'courses/:course_id/live_assessments/:assessment_id/results',
           action: :create,
           as: 'course_live_assessment_result_create'
    end
    ...
    scope(controller: 'support_helpers/plagiarism_platform') do
      ...
      get 'support_helpers/plagiarism_platform/resubmit_for_assignment/:assignment_id',
          action: :resubmit_for_assignment
    end
    ...
    scope(controller: :outcome_groups_api) do
      %w[global account course].each do |context|
        ...
        unless context == 'global'
          ...
          get "#{prefix}/outcome_group_links",
              action: :link_index,
              as: "#{context}_outcome_group_links"
        end
        get "#{prefix}/root_outcome_group", action: :redirect, as: "#{context}_redirect"
        get "#{prefix}/outcome_groups/account_chain",
            action: :account_chain,
            as: "#{context}_account_chain"
        ...
        get "#{prefix}/outcome_groups/:id/outcomes",
            action: :outcomes,
            as: "#{context}_outcome_group_outcomes"
        get "#{prefix}/outcome_groups/:id/available_outcomes",
            action: :available_outcomes,
            as: "#{context}_outcome_group_available_outcomes"
        post "#{prefix}/outcome_groups/:id/outcomes", action: :link
        put "#{prefix}/outcome_groups/:id/outcomes/:outcome_id",
            action: :link,
            as: "#{context}_outcome_link"
        delete "#{prefix}/outcome_groups/:id/outcomes/:outcome_id", action: :unlink
        get "#{prefix}/outcome_groups/:id/subgroups",
            action: :subgroups,
            as: "#{context}_outcome_group_subgroups"
        post "#{prefix}/outcome_groups/:id/subgroups", action: :create
        post "#{prefix}/outcome_groups/:id/import",
             action: :import,
             as: "#{context}_outcome_group_import"
        post "#{prefix}/outcome_groups/:id/batch",
             action: :batch,
             as: "#{context}_outcome_group_batch"
      end
    end
    ...
    scope(controller: :group_categories) do
      resources :group_categories, except: %i[index create]
      ...
      get 'group_categories/:group_category_id/export',
          action: :export,
          as: 'group_category_export',
          defaults: {
            format: :csv
          }
      post 'group_categories/:group_category_id/assign_unassigned_members',
           action: 'assign_unassigned_members',
           as: 'group_category_assign_unassigned_members'
    end
    ...
    scope(controller: :app_center) do
      %w[course account].each do |context|
        ...
        get "#{prefix}/apps/:app_id/reviews",
            action: :reviews,
            as: "#{context}_app_center_app_reviews"
        get "#{prefix}/apps/:app_id/reviews/self",
            action: :review,
            as: "#{context}_app_center_app_review"
    ...
    scope(controller: :conferences) do
      %w[course group].each do |context|
        ...
        post "#{prefix}/:conference_id/recording_ready",
             action: :recording_ready,
             as: "#{context}_conferences_recording_ready"
      end
    ...
    scope(controller: :custom_gradebook_column_data_api) do
      ...
      put 'courses/:course_id/custom_gradebook_column_data',
          action: :bulk_update,
          as: 'course_custom_gradebook_column_bulk_data'
    end
    ...
    scope(controller: :epub_exports) do
      get 'courses/:course_id/epub_exports/:id', { action: :show }
      get 'epub_exports', { action: :index }
      post 'courses/:course_id/epub_exports', { action: :create }
    end
    ...
    scope(controller: :grading_period_sets) do
      get 'accounts/:account_id/grading_period_sets',
          action: :index,
          as: :account_grading_period_sets
      post 'accounts/:account_id/grading_period_sets', action: :create
      patch 'accounts/:account_id/grading_period_sets/:id',
            action: :update,
            as: :account_grading_period_set
    ...
    scope(controller: :grading_periods) do
      ...
      put 'courses/:course_id/grading_periods/:id',
          action: :update,
          as: :course_grading_period_update
      delete 'courses/:course_id/grading_periods/:id',
             action: :destroy,
             as: :course_grading_period_destroy
      delete 'accounts/:account_id/grading_periods/:id',
             action: :destroy,
             as: :account_grading_period_destroy
    ...
    scope(controller: :errors) { post 'error_reports', action: :create }
    ...
    scope(controller: :inst_access_tokens) { post 'inst_access_tokens', action: :create }

    scope(controller: :gradebook_settings) do
      put 'courses/:course_id/gradebook_settings',
          action: :update,
          as: :course_gradebook_settings_update
    end
    ...
    scope(controller: :rubric_assessment_api) do
      post 'courses/:course_id/rubric_associations/:rubric_association_id/rubric_assessments',
           controller: :rubric_assessments,
           action: :create
      put 'courses/:course_id/rubric_associations/:rubric_association_id/rubric_assessments/:id',
          controller: :rubric_assessments,
          action: :update
      delete 'courses/:course_id/rubric_associations/:rubric_association_id/rubric_assessments/:id',
             controller: :rubric_assessments,
             action: :destroy
    end
    ...
    scope(controller: 'master_courses/master_templates') do
      ...
      get 'courses/:course_id/blueprint_templates/:template_id/associated_courses',
          action: :associated_courses,
          as: :course_blueprint_associated_courses
      put 'courses/:course_id/blueprint_templates/:template_id/update_associations',
          action: :update_associations
      get 'courses/:course_id/blueprint_templates/:template_id/unsynced_changes',
          action: :unsynced_changes,
          as: :course_blueprint_unsynced_changes

      post 'courses/:course_id/blueprint_templates/:template_id/migrations',
           action: :queue_migration
      get 'courses/:course_id/blueprint_templates/:template_id/migrations',
          action: :migrations_index,
          as: :course_blueprint_migrations
      get 'courses/:course_id/blueprint_templates/:template_id/migrations/:id',
          action: :migrations_show
      get 'courses/:course_id/blueprint_templates/:template_id/migrations/:id/details',
          action: :migration_details

      put 'courses/:course_id/blueprint_templates/:template_id/restrict_item',
          action: :restrict_item

      get 'courses/:course_id/blueprint_subscriptions',
          action: :subscriptions_index,
          as: :course_blueprint_subscriptions
      get 'courses/:course_id/blueprint_subscriptions/:subscription_id/migrations',
          action: :imports_index,
          as: :course_blueprint_imports
      get 'courses/:course_id/blueprint_subscriptions/:subscription_id/migrations/:id',
          action: :imports_show
      get 'courses/:course_id/blueprint_subscriptions/:subscription_id/migrations/:id/details',
    ...
    scope(controller: :content_shares) do
      ...
      get 'users/:user_id/content_shares/sent',
          action: :index,
          defaults: {
            list: 'sent'
          },
          as: :user_sent_content_shares
      get 'users/:user_id/content_shares/received',
          action: :index,
          defaults: {
            list: 'received'
          },
          as: :user_received_content_shares
    ...
    scope(controller: :history) { get 'users/:user_id/history', action: 'index', as: :user_history }

    scope(controller: :gradebooks) do
      put 'courses/:course_id/update_final_grade_overrides', action: 'update_final_grade_overrides'
      put 'courses/:course_id/apply_score_to_ungraded_submissions',
          action: 'apply_score_to_ungraded_submissions'
    end
    ...
    scope(controller: 'course_pacing/section_paces_api') do
      ...
      post 'courses/:course_id/sections/:course_section_id/paces',
           action: :create,
           as: :new_section_pace
      patch 'courses/:course_id/sections/:course_section_id/pace',
            action: :update,
            as: :patch_section_pace
      delete 'courses/:course_id/sections/:course_section_id/pace',
             action: :delete,
             as: :delete_section_pace
    end

    scope(controller: 'course_pacing/student_enrollment_paces_api') do
      get 'courses/:course_id/student_enrollments/:student_enrollment_id/pace',
          action: :show,
          as: :student_enrollment_pace
      post 'courses/:course_id/student_enrollments/:student_enrollment_id/paces',
           action: :create,
           as: :new_student_enrollment_pace
      patch 'courses/:course_id/student_enrollments/:student_enrollment_id/pace',
            action: :update,
            as: :patch_student_enrollment_pace
      delete 'courses/:course_id/student_enrollments/:student_enrollment_id/pace',
             action: :delete,
             as: :delete_student_enrollment_pace
    end
  ...
  ApiRouteSet::V1.draw(self) do
    ...
    scope(controller: :jobs_v2) do
      ...
      get 'jobs2/:bucket',
          ...
          as: :jobs_list,
          constraints: {
            bucket: /running|queued|future|failed/
          }
  ...
  get 'post_message_forwarding',
      controller: 'lti/platform_storage',
      action: :post_message_forwarding,
      as: :lti_post_message_forwarding

  ApiRouteSet.draw(self, '/api/lti/v1') do
    post 'tools/:tool_id/grade_passback',
         controller: :lti_api,
         action: :grade_passback,
         as: 'lti_grade_passback_api'
    post 'tools/:tool_id/ext_grade_passback',
         controller: :lti_api,
         action: :legacy_grade_passback,
         as: 'blti_legacy_grade_passback_api'
    ...
    post 'logout_service/:token',
         controller: :lti_api,
         action: :logout_service,
         as: 'lti_logout_service'
    post 'turnitin/outcomes_placement/:tool_id',
         controller: :lti_api,
         action: :turnitin_outcomes_placement,
         as: 'lti_turnitin_outcomes_placement'
  end
  ...
  ApiRouteSet.draw(self, '/api/lti') do
    scope(controller: 'lti/tool_configurations_api') do
      ...
      %w[account course].each do |context|
        get "#{context}s/:#{context}_id/developer_keys/:developer_key_id/tool_configuration",
            action: :show,
            as: "#{context}_show_tool_configuration"
      end
    end
    ...
    %w[course account].each do |context|
      ...
      post "#{prefix}/authorize",
           controller: 'lti/ims/authorization',
           action: :authorize,
           as: "#{context}_lti_oauth2_authorize"
      ...
      get "tool_proxy/:tool_proxy_guid/#{prefix}/tool_setting",
          controller: 'lti/ims/tool_setting',
          action: :show,
          as: "show_#{context}_tool_setting"
      get "tool_proxy/:tool_proxy_guid/#{prefix}/resource_link_id/:resource_link_id/tool_setting",
          controller: 'lti/ims/tool_setting',
          action: :show,
          as: "show_#{context}_resource_link_id_tool_setting"
      put "tool_proxy/:tool_proxy_guid/#{prefix}/tool_setting",
          controller: 'lti/ims/tool_setting',
          action: :update,
          as: "update_#{context}_tool_setting"
      put "tool_proxy/:tool_proxy_guid/#{prefix}/resource_link_id/:resource_link_id/tool_setting",
          controller: 'lti/ims/tool_setting',
          action: :update,
          as: "update_#{context}_update_resource_link_id_tool_setting"
    end

    # Tool Setting Services
    get 'tool_settings/:tool_setting_id',
        controller: 'lti/ims/tool_setting',
        action: :show,
        as: :show_lti_tool_settings
    get 'tool_proxy/:tool_proxy_guid/tool_setting',
        controller: 'lti/ims/tool_setting',
        action: :show,
        as: :show_tool_proxy_lti_tool_settings
    put 'tool_settings/:tool_setting_id',
        controller: 'lti/ims/tool_setting',
        action: :update,
        as: :update_lti_tool_settings
    put 'tool_proxy/:tool_proxy_guid/tool_setting',
        controller: 'lti/ims/tool_setting',
        action: :update,
        as: :update_tool_proxy_lti_tool_settings

    # Tool Proxy Services
    get 'tool_proxy/:tool_proxy_guid',
        controller: 'lti/ims/tool_proxy',
        action: :show,
        as: 'show_lti_tool_proxy'

    # Membership Service
    get 'courses/:course_id/membership_service',
        controller: 'lti/membership_service',
        action: :course_index,
        as: :course_membership_service
    get 'groups/:group_id/membership_service',
        controller: 'lti/membership_service',
        action: :group_index,
        as: :group_membership_service
    ...
    scope(controller: 'lti/submissions_api') do
      ...
      get 'assignments/:assignment_id/submissions/:submission_id/attachment/:attachment_id',
          action: :attachment,
          as: :lti_submission_attachment_download
    end

    # Originality Report Service
    scope(controller: 'lti/originality_reports_api') do
      post 'assignments/:assignment_id/submissions/:submission_id/originality_report',
           action: :create
      put 'assignments/:assignment_id/submissions/:submission_id/originality_report/:id',
          action: :update
      put 'assignments/:assignment_id/files/:file_id/originality_report', action: :update
      get 'assignments/:assignment_id/submissions/:submission_id/originality_report/:id',
          action: :show
    ...
    scope(controller: 'lti/ims/scores') do
      post 'courses/:course_id/line_items/:line_item_id/scores',
           action: :create,
           as: :lti_result_create
    end

    # Result Service (LTI AGS)
    scope(controller: 'lti/ims/results') do
      get 'courses/:course_id/line_items/:line_item_id/results/:id',
          action: :show,
          as: :lti_result_show
    ...
    scope(controller: 'lti/account_external_tools') do
      post '/accounts/:account_id/external_tools',
           action: :create,
           as: :account_external_tools_create
      get '/accounts/:account_id/external_tools/:external_tool_id',
          action: :show,
          as: :account_external_tools_show
      get '/accounts/:account_id/external_tools', action: :index, as: :account_external_tools_index
      delete '/accounts/:account_id/external_tools/:external_tool_id',
             action: :destroy,
             as: :account_external_tools_destroy
    end
    ...
    scope(controller: 'lti/data_services') do
      ...
      get '/accounts/:account_id/event_types',
          action: :event_types_index,
          as: :data_services_event_types
      delete '/accounts/:account_id/data_services/:id', action: :destroy, as: :data_services_destroy
    end

    # Account Lookup service
    scope(controller: 'lti/account_lookup') { get '/accounts/:account_id', action: :show }

    # Names and Roles Provisioning (NRPS) v2 Service
    scope(controller: 'lti/ims/names_and_roles') do
      get 'courses/:course_id/names_and_roles',
          controller: 'lti/ims/names_and_roles',
          action: :course_index,
          as: :course_names_and_roles
      get 'groups/:group_id/names_and_roles',
          controller: 'lti/ims/names_and_roles',
          action: :group_index,
          as: :group_names_and_roles
    end

    # Security
    scope(controller: 'security') { get 'security/jwks', action: :jwks, as: :jwks_show }
    ...
    get 'advantage_token',
        controller: 'lti/token',
        action: :advantage_access_token,
        as: :lti_advantage_token_site_admin
  end

  ApiRouteSet.draw(self, '/api/sis') do
    scope(controller: :sis_api) do
      get 'accounts/:account_id/assignments',
          action: 'sis_assignments',
          as: :sis_account_assignments
    ...
    scope(controller: :disable_post_to_sis_api) do
      put 'courses/:course_id/disable_post_to_sis',
          action: 'disable_post_to_sis',
          as: :disable_post_to_sis_course_assignments
    end
  end
end
